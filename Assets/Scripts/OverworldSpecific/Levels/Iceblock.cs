    ...
    void Update()
    {
        if (isMoving)
        {
            ...
            if (Physics2D.Raycast(transform.position, moveVector, 0.01f, holelayerMask))
            {
                ...
                FindObjectOfType<OverworldManager>().GetComponent<AudioManager>().PlayTrack(AUDIOCLIPS.PUZZLESOLVED);
            }
            ...
            if (!hasPlayed)
            {
                FindObjectOfType<OverworldManager>().GetComponent<AudioManager>().PlayTrack(AUDIOCLIPS.STONEMOVE);
                hasPlayed = true;
            }
